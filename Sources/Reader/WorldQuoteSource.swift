// WorldQuoteSource.swift — the world indices, from Yahoo Finance's chart endpoint.
//
...
struct WorldQuoteSource: QuoteSource {
    ...
    private func chart(_ symbol: String) async throws -> Chart {
        let wanted = WorldIndex.yahooSymbol(for: symbol)
...
private extension Result {
    /// The error, for a batch that needs to tell "everything failed" from "one symbol isn't listed".
    var failure: Failure? {
        switch self {
